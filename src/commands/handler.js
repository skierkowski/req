...
export const executeCommand = async (commandName, cliParams, options = {}) => {
  const { configFile = 'req.yaml', verbose = false, stage = 'default' } = options;
  ...
  if (verbose && Object.keys(stageVariables).length > 0) {
    console.log(`Using stage '${stage}' variables:`, JSON.stringify(stageVariables, null, 2));
  }
  ...
  const responseData = await makeRequest(resolvedConfig.url, requestOptions, verbose);
  
  // Extract value using the configured path (default to "$.text")
  const valuePath = resolvedConfig.value || '$.text';
